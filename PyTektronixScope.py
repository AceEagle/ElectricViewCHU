...
class TektronixScope(object):
    ...
    be made as any Visa Instrument :  scope.query('*IDN?')
    """    
    def __init__(self, inst):
        """ Initialise the Scope

        argument : 
            inst : should be a string or an object with write and query method
    ...
    def query_raw(self, cmd):
        if hasattr(self._inst, 'query_raw'):
            return self._inst.query_raw(cmd)[:-1]
        else:
            return self._inst.query(cmd)
...
should be in %s"%(str(name), ' '.join(channel_list)))
    ...
    def is_channel_selected(self, channel):
        return self.query('SEL:%s?'%(self.channel_name(channel)))=='1'

    def get_channel_offset(self, channel):
        return float(self.query('%s:OFFS?'%self.channel_name(channel)))

    def get_channel_position(self, channel):
        return float(self.query('%s:POS?'%self.channel_name(channel)))

    def get_vertical_scale(self, channel):
        return float(self.query('%s:SCA?'%self.channel_name(channel)))
    ...
    def get_impedance(self, channel):
        ...
        return self.query('%s:IMPedance?'%self.channel_name(channel))
    ...
    def get_coupling(self, channel):
        ...
        return self.query('%s:COUPling?'%self.channel_name(channel))
...
# Waveform Transfer Command Group
    ...
    def get_out_waveform_horizontal_zero(self):
        return float(self.query('WFMO:XZERO?'))

    def get_out_waveform_vertical_scale_factor(self):
        return float(self.query('WFMO:YMUlt?'))

    def get_out_waveform_vertical_position(self):
        return float(self.query('WFMO:YOFf?'))
